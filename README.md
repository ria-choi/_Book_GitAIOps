...
이 저장소는 [AI 시대에 개발자가 알아야 할 인프라 구성 배포 with 클로드 코드](https://product.kyobobook.co.kr/detail/S000220220936) 책의 실습을 위한 코드를 제공합니다.
...
├── AGENTS.md           # Codex CLI 한정 차이점 어댑터
├── GEMINI.md           # Gemini CLI 한정 차이점 어댑터
└── README.md
```
...
이 책은 **Claude Code**를 기준으로 쓰였지만, 같은 가드레일 시스템이 다른 AI 에이전트에서도 동작합니다.

| 에이전트 | 진입점 파일 | 실행 명령 |
|---------|-----------|---------|
| **Claude Code** (기준) | `CLAUDE.md` (자동 로드) | `claude --dangerously-skip-permissions` |
| **Codex CLI** | `AGENTS.md` | `codex --full-auto --sandbox danger-full-access` |
| **Gemini CLI** | `GEMINI.md` | `gemini --yolo --model gemini-2.5-pro` |

각 어댑터 파일에는 해당 에이전트의 제약과 차이점만 담겨 있습니다. `decision-guides/`, `prompt-guardrails/`, `result-templates/` 세 디렉터리는 어느 에이전트에서나 동일하게 동작합니다.
...
✔️ 조 훈 ( [GitHub](https://github.com/sysnet4admin) · [LinkedIn](https://linkedin.com/in/hoonjo) · [YouTube](https://youtube.com/@KubernetesLab) · [Tech Blog](https://kuberneteslab.dev/ko) )

## 도서 구입 안내

본 도서는 각 온오프라인 서점에서 만나보실 수 있습니다. (ISBN 979-11-407-1917-4)

- 📍  [교보문고](https://product.kyobobook.co.kr/detail/S000220220936)
- 📍  [예스24](https://link.yes24.com/a/Ldoo2neRav)
- 📍  [알라딘](https://www.aladin.co.kr/shop/wproduct.aspx?ISBN=9791140719174)
